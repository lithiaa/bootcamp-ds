...
async function compareScreenshots(driver, imageName, maxDiffPercent = 2) {
  ...
  const diffPath = path.join(diffDir, imageName + "_diff.png");
  ...
  if (diffPercent > maxDiffPercent) {
    ...
    throw new Error(
      `Visual regression detected! Difference: ${diffPercent.toFixed(2)}% exceeds threshold of ${maxDiffPercent}% threshold.`,
    );
  } else {
